...
class SaquesModel extends Model
{
    ...
    public static function addSaque(SaqueVO $saque)
    {
        ...
        if (DadosModel::get()->getSaldoInicial()) {
            $saldo = $user->getCredito() - $user->getSaldoInicial();
        } else {
...
//        } # Indicados
        ...
        else if ($saque->getValor() > $saldo) {
            throw new \Exception('Saldo insuficiente para saque. Seu saldo atual é de R$ ' . Number::real($saldo));
        } # Saque mínimo
        else if ($saque->getValor() < 50) {
            throw new \Exception('Saque mínimo de R$ 50,00');
        }

        if ($taxa > 0) {
            $saque->setTaxa($taxa);
            if (($saque->getValor() + $taxa) > $saldo) {
