...
class HistoricoBancarioModel extends Model
{
    ...
    static function busca(array $parans = null, $page = 1, $forPage = 10)
    {
            ...
            foreach ($parans as $key => $value) {
                if (!isEmpty($value) and !empty($key)) {
                    switch ($key) {
                        case 'situacao':
                            switch ($value) {
                                case 'premio-invalido':
                                    $query->addInnerJoin(ApostasModel::getTable(), 'aposta', "aposta.id = a.refid AND a.ref = :situacaoTable AND aposta.ganhou = 0 AND a.status = 1 AND a.estornada = 0 AND a.reftype = 'premio'");
                                    $places['situacaoTable'] = ApostasModel::getTable();
                                    break;
                            }
                            break;
                        case 'horaInicial':
        ...
        $query->setWhere($termos)->setOrder($order);

        return self::listaPagination($query, $places, $page, $forPage);
    }
