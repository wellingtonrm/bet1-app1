...
abstract class Model
{
     ...
     */
    public static function save(ValueObject $vo, bool $soAlterados = true)
    {
     ...
     * @return boolean
     */
    public static function excluir($VO_or_ID = null)
    {
        try {
            ...
            if (is_a($Value, ValueObject::class)) {
                if ($Value->getTable() != self::getTable()) {
                    throw new Exception('Inválido: ' . $Value->getTable() . ' != ' . self::getTable());
                } else if (!$Value->getId()) {
            ...
            # Excluíndo
            self::delete(self::getTable(), 'WHERE `id` = :id LIMIT 1', ['id' => $id]);
