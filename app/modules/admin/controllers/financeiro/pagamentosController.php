...
class pagamentosController extends Controller
{
    ...
    function save(PagamentoVO $v)
    {
        try {
            ...
            if ($v->getData() <= date('Y-m-d')) {
                $v->setPago(1)->setDataPagamento($v->getData())->save();
            }

            HistoricoModel::add($novo ? "Criou o pagamento #{$v->getId()}" : "Alterou o pagamento #{$v->getId()}", $v, Admin::getLogged());

            if ($novo and $v->voUser()) {
                $descricao = $v->voDescricao();
                HistoricoBancarioModel::add($v->voUser(), -$v->getValor(), $descricao ? $descricao->getTitle() : 'Novo pagamento #' . $v->getId(), $v, "pagamento");
            }
