    ...
    function excluirAction()
    {
        try {
            ...
            if ($aposta->getStatus() == ApostasModel::STATUS_ATIVA) {
                ...
                # Comissão de aposta
                if ($aposta->getComissao() and $aposta->voUser()) {
                    HistoricoBancarioModel::add($aposta->voUser(), -$aposta->getComissao(), "Extorno de comissão de aposta #{$aposta->getId()}", $aposta, 'cancelamento');
                }

                # Comissão de Gerente
                if ($aposta->getComissaoGerente() and $aposta->voGerente()) {
                    HistoricoBancarioModel::add($aposta->voGerente(), -$aposta->getComissaoGerente(), "Extorno de comissão de aposta #{$aposta->getId()}", $aposta, 'cancelamento');
                }
...
SET a.databaixa = :hoje, a.update = :update, a.pago = 1
